...
Pierwsza z modyfikacji oryginalnego algorytmu dodaje mechanizm \textit{pivoting selection} opisany pierwotnie w pracy \cite{pivot} (linia 9). Zgodnie z zasadą wyszukiwania wzorców maksymalnych kolokacji, może on zawierać wierzchołek $ u $ lub niesąsiadujące z nim wierzchołki - jeżeli nie zawiera, zostanie on dodany (linie 11-13). W związku z tym, w linii 10 następuje iteracja tylko po wierzchołku $ u $ i wszystkich innych wierzchołkach nie będących jego sąsiadami, aby ograniczyć liczbę węwnętrznych pętli w rekurencji.
