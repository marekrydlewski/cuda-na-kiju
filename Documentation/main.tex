...
wybór pivota $ u \in M \cup T ; \% $ do maksymalizacji $ | M \cap N(u) | $\;
...
Pierwsza z modyfikacji oryginalnego algorytmu dodaje mechanizm \textit{pivoting selection} opisany pierwotnie w pracy \cite{pivot} (linia 9). Wybiera on wierzchołek będący osią podziału zbioru (tzw. \textit{pivot} w oparciu o rozmiar unii sąsiadów tego wierzchołka i wierzchołków kandydatów. Każda maksymalna klika musi zawierać albo wierzchołek $ u $, albo niesąsiadujące z nim wierzchołki - jeżeli nie zawiera, zostanie on dodany (linie 11-13). W związku z tym, tylko wierzchołek $ u $ i jego nie-sąsiedzi muszą być przetestowani (linia 10).

Druga modyfikacja opiera się o pojęcie rozproszenia grafu. Opisuje się je miarą \textit{degeneracji grafu} \cite{degenerat}:
...
Wierzchołki w zewnętrznej rekurencji są uporządkowane według stopnia degeneracji (linia 2). W ciele rekurencji (linie 3-5) liczba wierzchołków czekających na weryfikację nie przekroczy $ k $. Tym sposobem ograniczono liczbę zewnętrznych rekurencji.
...
Na tym etapie odbywa się generowanie dwuelementowych kandydatów na kolokacje i ich filtrowanie na podstawie progu odległości. Wykorzystano tutaj podejście tzw. \textit{wymiatania} (ang. \textit{plane sweep algorithm}) które jest paradygmatem algorytmów wykorzystywanych do rozwiązywania różnych problemów w przestrzeni euklidesowej. W zastosowanej implementacji można wyróżnić następujące kroki:
\begin{itemize}
\item Sortowanie wektora \lstinline{std::vector<DataFeed> source} zawierającego dane wejściowe. Wykorzystano tutaj sortowanie szybkie z biblioteki \lstinline{<algorithm>}. Kryterium sortowania jest tutaj relacja między arbitralnie wybranym wymiarem przestrzeni.
\item Wyliczanie \textit{efektywnego progu odległości} przez podniesienie progu odległości do kwadratu - dzięki temu można uniknąć kosztownego pierwiastkowania przy obliczaniu odległości między dwoma punktami. Jest to możliwe, gdyż potrzebna jest tylko informacja o relacji między ową odległością a progiem odległości. Zbędna jest wiedza o rzeczywistej odległości między punktami.
\item Dla każdego elementu danych iterujemy w wybranym kierunku sprawdzając odległość między tym punktem a aktualnie iterowanym, w przypadku gdy odległość jest mniejsza od progowej umieszczamy taki element na odpowiednim miejscu w \lstinline{insTable}. Główną ideą jest tutaj przerwanie wewnętrznej pętli gdy wartość bezwzględna różnicy posortowanych współrzędnych owych punktów jest większa od progu odległości tj. \textit{tutaj matematyczny wzorek}. Zajście takiej sytuacji gwarantuje, że odległość między każdym kolejnym punktem będzie większa od progu odległości, stąd możliwość przerwania pętli. 

W odróżnieniu od pracy \cite{chinczyki}, w której zaproponowano dwuwymiarową hashmapę, \lstinline{insTable} zawierająca dwuelementowych kandydatów na kolokacje ma postać trójwymiarowej mapy wskaźników na wektor liczb:
\begin{lstlisting}
std::unorderd_map<unsigned short, std::unordered_map<unsigned short,
	std::unordered_map<unsigned short, std::vector<
    	unsigned short>*>>> insTable;
\end{lstlisting}
 Pierwsze dwa wymiary określają typy elementów \textit{A} i \textit{B}, natomiast trzeci wymiar określa numer instancji przestrzennej \textit{A}. W wektorze przechowywane są kolejne numery instancji przestrzennej \textit{B}. Dodatkowo, dane są umieszczane w ten sposób że zawsze spełniona jest relacja \textit{tutaj matematyczny znaczek: numer cechy A \textless numer cechy B}. Taka implementacja umożliwia łatwe i efektywne odczytywanie sąsiadów o danej cesze konkretnej instancji przestrzennej. Wykorzystanie \lstinline{std::unordered_map} zamiast \lstinline{std::map} pomimo większego zużycia pamięci znacząco (kilkukrotnie) zwiększa wydajność przeglądania map. 
 
 \item Korzystając z faktu, że w trakcie działania metody następuje iteracja po wszystkich elementach, równocześnie tworzony jest wektor \lstinline{std::vector<unsigned short> typeIncidenceCounter}, w którym zliczane są wystąpienia kolejnych cech instancji przestrzennych.
