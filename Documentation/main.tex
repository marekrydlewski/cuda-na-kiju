...
Na początku wykonywana jest funkcja $ countUniqueInstances$. Ma ona na celu zliczenie wszystkich wystąpień par instancji bez duplikatów. Podobnie jak w kroku pierwszym zostały tu użyte mapy nieuporządkowane (\textit{std::unordered\_map}) w celu przyspieszenia przetwarzania. Do budowy mapy, której kluczem są pary lub krotki konieczne jest zastosowanie funkcji mieszającej. Ogranicza ona występowanie kolizji w przypadku wstawiania nowych elementów do tablicy z haszowaniem. Popularną metodą jest skorzystania z funkcji XOR i zastosowanie jej do dających się pojedynczo hashować elementów pary. Ze względu na to, że jednak funkcja ta generuje niezadowalająco dużą ilość kolizji, funkcja mieszająca powstała na bazie \textit{hash\_combine} z biblioteki \textit{boost}.
...
$ seed_1 \leftarrow 0 $\;
$ seed_1 \leftarrow hash_combine(seed_1, p.first) $
$ seed_1 \leftarrow hash_combine(seed_1, p.second) $

$ seed_2 \leftarrow 0 $\;
$ seed_2 \leftarrow hash_combine(seed_2, p.second) $
$ seed_2 \leftarrow hash_combine(seed_2, p.first) $

$ seed \leftarrow minimum(seed_1, seed_2) $

\caption{Funkcja hashująca dla pary $p$}
\label{alg:pairHashFunction}
\end{algorithm}

% do przerobienia na algo
...
Tak jak w przypadku oryginalnego algorytmu, kandydaci na kolokacje maksymalne są generowani w oparciu o \textit{graf kolokacji o rozmiarze 2} (patrz Definicja~\ref{def:size2-col-graph}). Graf ten tworzony za pomocą funkcji \textit{createSize2ColocationsGraph}, wykorzystuje macierz sąsiedztwa (ang. \textit{adjacency matrix}) i jest oparty na bazie struktury $ insTable $. Krawędź między elementami pary instancji jest tworzona tylko i wyłącznie wtedy, kiedy numer instancji przestrzennej $ A $ jest większy od zera.
...
Dla klik o długości większej niż 2 tworzone jest skondensowane drzewo instancji (patrz Definicja~\ref{def:condensed}). Implementacja konstrukcji takiego drzewa zasadniczo nie różni się od pierwotnego algorytmu zaprezentowanego w pracy \cite{chinczyki}, zastosowano w nim jednak pewne metody optymalizacyjne - wskaźniki do rodziców w celu ograniczenia przeszukiwań drzewa w dół, użycie wektora zawierającego wskaźniki do liści znajdujących się na ostatnim poziomie drzewa wynikającym z rozmiaru aktualnie budowanej kliki - w przypadku gdy nie udało się rozbudować drzewa o kolejny poziom lista ta jest pusta.. Takie podejście umożliwia szybki dostęp do finalnych instancji poszczególnych kandydatów na kolokacje.  Następnie zliczane są wystąpienia instancji w klikach za pomocą odwróconej pętli i na ich podstawie wyliczana jest miara powszechności (najpierw iterujemy po poszczególnych cechach kolejnych wektorów).
...
W porównaniu do wersji sekwencyjnej, konstrukcja skondensowanego drzewa instancji oraz liczenie miary powszechności jest dokonywany w sposób równoległy. Wymagało to zaprojektowania równoległych odpowiedników kontenerów znanych z odmiany sekwencyjnej, czyli \textit{ParallelCliqueContainer} i \textit{ParallelLapsedCliqueContainer} zapewniających bezpieczeństwo przetwarzania równoległego. Cel ten osiągnięto za pomocą skorzystania z sekcji krytycznych w przypadku inkrementowania liczników jak i posłużenia się współbieżnymi strukturami danych z biblioteki PPL, co przełożyło się również na zwiększenie efektywności przetwarzania. Niezbędne było także zastosowanie (podobnie jak w kroku pierwszym) dynamicznego planisty (ang. \textit{auto scheduler}) przydzielającego dynamicznie kolejne kliki do przetworzenia do poszczególnych wątków. Niemożliwym było zastosowanie statycznego planisty z uwagi na fakt, że na tym etapie nie można przewidzieć dokładnego czasu konstrukcji skondensowanego drzewa instancji. Dla \textit{ParallelCliqueContainera} konieczne okazało się zaimplementowanie funkcji mieszającej dla całych wektorów - struktura równoległego zbioru (ang. \textit{concurrent set}) w odróżnieniu od swojego odpowiednika z biblioteki STD wykorzystuje hashe zamiast korzystać z relacji porównania wkładanych do niego elementów. Również w tym przypadku skorzystano z funkcji \inline{hash_combine}. %tutaj przypis do algo hash combine
...
Poniżej przedstawiono wykresy ilustrujące zmienność czasu działania algorytmu w różnych implementacjach ze względu na zmienny próg maksymalnej odległości.
...
\begin{axis}[
            ...
            ymin=0, ymax=135000,
            xmin=2, xmax=6,
            restrict y to domain=0:2500000,
           ymajorgrids=true,
           grid style=dashed,
           width=0.95\textwidth,
           height=8cm,
...
Poniżej przedstawiono wykres ilustrujący zmienność czasu działania algorytmu w różnych implementacjach ze względu na zmienny próg minimalnej powszechności.
