...
Dla rzadkiego zbioru danych implementacja GPU jest generalnie wolniejsza od implementacji na CPU, szczególnie od implementacji równoległej co można zauważyć głównie dla niższych wartości progu maksymalnej odległości.
\newline

W przypadku gęstego zbioru danych implementacja na GPU sprawuje się lepiej, od pewnego punktu prześcigając obie implementacje na CPU.
\begin{figure}[H]
\begin{tikzpicture}
\begin{axis}[
            legend style={at={(0.5,-0.25)},anchor=north,legend cell align=left},
...
Dla rzadkich danych których część jest zgrupowana w skupiska implementacja na GPU wypada nadzwyczaj słabo w porównaniu do implementacji na CPU.
\begin{figure}[H]
\begin{tikzpicture}
\begin{axis}[
            legend style={at={(0.5,-0.25)},anchor=north,legend cell align=left},
...
W przypadku gdy dane są gęste oraz zgrupowane w skupiska wydajność poszczególnych implementacji jest analogiczna jak dla danych gęstych w których nie występują skupiska. Wynika to z tego, że przy dużym zagęszczeniu danych skupienie ich części w pewnych sektorach nie powoduje znacznego zmniejszenia gęstości poza nimi, więc ogólny rozkład danych w dalszym ciągu jest dość równomierny.
...
Zmienność progu minimalnej powszechności nie ma większego wpływu na zależności pomiędzy czasami wykonań poszczególnych implementacji - implementacja GPU pozostaje wolniejsza niż implementacja równoległa na CPU oraz szybsza niż implementacja sekwencyjna. Zmienia się to jedynie w przypadku wysokich progów powszechności (powyżej 0.3) - implementacja GPU jest wtedy wolniejsza również od implementacji sekwencyjnej. Wynika to z tego, że przy takim progu minimalnej powszechności odfiltrowana zostaje większość danych wejściowych, przez co implementacje CPU zostają "bezrobotne", lecz implementacja GPU w dalszym ciągu musi wykonać wszelkie działania związane z kopiowaniem danych na kartę graficzną oraz przygotowaniem równoległego wykonania algorytmu.

\subsection{Wnioski}

Mimo tego, że implementacja GPU w ogólności przypadków wygląda na wolniejszą niż implementacja równoległa na CPU, zdaje się być również bardziej efektywna dla danych o dużej gęstości i dużym rozmiarze.
 Implementacja sekwencyjna na CPU niemal w każdym przypadku wykazuje się niższą efektywnością niż pozostałe implementacje.
