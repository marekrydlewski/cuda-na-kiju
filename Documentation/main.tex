...
Ostatni krok implementacji algorytmu można podzielić na cztery mniejsze kroki

\begin{enumerate}
	\item pobranie ze zbioru \textit{uniqueCandidates} zbioru najdłuższych, nieprzetworzonych kandydatów oraz wprowadzenie ich do pamięci GPU. Zbiór ten oznaczmy jako \textit{toProcess}
	\item wygenerowanie drzewa instancji oraz na podstawie tego drzewa zmaterializowanie wszystkich unikalnych instancji aktualnie przetwarzanych kandydatów
	\item obliczenie prewalentności przetwarzanych kandydatów 
	\item w zależności od wartości współczynnika prewalentności zaliczenie kandydata do zbioru kolokacji maksymalnych, lub rozbicie go na \textit{n} krótszych o 1 unikalnych kandydatów 
\end{enumerate}

kroki te są powtarzane w cyklu do momentu w którym w zbiorze \textit{candidates} nie ma już nieprzetworzonych kandydatów. Dodatkowo ze względu na ograniczenia pamięciowe przestrzeni GPU zbiór \textit{toProcess} dzielimy na podzbiory (których wielkość zależy od ilości pamięci), kroki II oraz III są następnie wykonywane dla każdego takiego podzbioru osobno.

Kandydaci są przetwarzani grupami, jedna grupa to jedna iteracja, w każdej iteracji przetwarzani kandydaci mają długość o jeden mniejszą niż w poprzedniej iteracji. Kolejność ta jest ważna, ponieważ zapewnia nam, że aktualnie przetwarzany kandydat nie jest częścią większej kliki. 

Na początku każdej iteracji pobieramy wektor kandydatów znajdujący się pod kluczem równym aktualnej długości przetwarzanych kandydatów. Wektor ten następnie umieszczamy w pamięci GPU.

Celem kroku drugiego jest materializacja kandydatów co posłuży do obliczenia powszechności ich występowania. Do tego celu użyte zostało drzewo instancji. Każdy \textit{i}-ty poziom drzewa instancji odpowiada \textit{i}-temu elementowi kandydata.
Pierwsze dwa poziomy budowane są na podstawie 
