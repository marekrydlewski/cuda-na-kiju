	...
	namespace Foxtrot
	{
		...
		*/
		...
		__host__ void PlaneSweep(
			...
			, std::shared_ptr<GPUHashMapper<UInt, Ptx, GPUKeyProcessor<UInt>>>& resultHashMap
			...
			, thrust::device_vector<Idx>& resultPairsB)
		{
			...
			typedef thrust::device_vector<Idx> InstancesDeviceVector;
			...
			resultHashMap.reset(new GPUHashMapper<UInt, Ptx, GPUKeyProcessor<UInt>>(
