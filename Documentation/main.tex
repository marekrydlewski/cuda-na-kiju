...
Oddzielną kwestią jest literatura książkowa, wykorzystana do zapoznania się z technologią CUDA oraz przyjęcia dobrych praktyk optymalizacyjnych i programistycznych. Tutaj szczególnie należy wymienić popularną pozycję \textit{CUDA w przykładach} autorstwa Shane'a Cooke'a \cite{cuda_by_examples}, a także \textit{Professional CUDA C Programming} \cite{professional_cuda} będącą również podstawą do wstępu teoretycznego w Rozdziale 3.

\subsection{Struktura pracy}

\textbf{Rozdział 2} stanowi wstęp teoretyczny do niniejszej pracy - prezentuje podstawowe pojęcia związane z odkrywaniem kolokacji przestrzennych, a także prezentuje najprostsze algorytmy wyszukujące wzorce kolokacji przestrzennych.

\textbf{Rozdział 3} zawiera wprowadzenie do technologii CUDA. Przedstawia także podstawowe różnice w programowaniu na CPU i GPU oraz wyjaśnia podstawowe pojęcia związane z programowaniem na procesory graficzne.

\textbf{Rozdział 4} poświęcony jest algorytmowi \textit{SCGT} będącym głównym tematem pracy. Zawiera wyjaśnienia poszczególnych kroków algorytmu oraz niezbędne definicje.

Implementacja CPU (w wersji sekwencyjnej, jak i równoległej) jest głównym tematem \textbf{Rozdziału 5}. Koncentruje się on w szczególności na przedstawienie zastosowanych technik optymalizacyjnych poprawiających wydajność algorytmu w środowisku CPU. Rozdział zawiera także krótki wstęp do biblioteki \textit{PPL} użytej w równoległej implementacji algorytmu.

\textbf{Rozdział 6} zawiera omówienie implementacji algorytmu dla procesorów graficznych (GPU).

Porównanie wydajności zaimplementowanych rozwiązań zostało zawarte w \textbf{Rozdziale 6}. 

Ostatni \textbf{Rozdział 7} stanowi podsumowanie całego projektu, uwagi zespołu oraz możliwe dalsze kierunki rozwoju.

W \textbf{Dodatku A} zawarty jest krótki opis klas zaimplementowanych w implementacji dla CPU i GPU.
...
Krok ten, tak samo jak w wersji sekwencyjnej stanowi zmodyfikowany algorytm obliczania miar powszechności znanego z algorytmu \textit{Co-location Miner} (patrz Rozdział 2).
...
Na wejściu tego kroku oczekujemy wczytanej instancji danych oraz parametru \textit{distanceTreshold} zawierającego wartość maksymalnej odległości pomiędzy instancjami. Na wyjściu otrzymujemy dwie tablice przechowujące pary instancji dla których przy założonym zachodzi rela

Algorytm wybrany do rozwiązania tego zadania to algorytm \textit{Planesweep} przystosowany do uruchamiania w architekturze masowo równoległej. Sam algorytm podzielić możemy na dwie części, wyznaczanie ilości sąsiadów dla każdej instancji, a następnie alokacja i wpisanie wartości do dwóch tabel \textit{InstanceFirst} oraz \textit{InstanceSecond} które służą do przechowywania par instancji będących w relacji sąsiedztwa, gdzie pierwsza tabela zawiera elementy będące pierwsze w parze, a druga pozostałe. Wartości należące do jednej pary znajdują się pod tymi samymi indeksami. Kolejność wyznaczana jest przez następujący operator
...
Pierwszą czynnością jest posortowanie zbioru danych wejściowych względem osi odciętych. Dzięki temu ograniczymy obszar przeszukiwań sąsiadów instancji. 

TODO planesweep count

TODO planesweep fill

Ostatnią czynnością jest posortowanie par instancji przy użyciu następującego operatora. Przyczyna takiego uporządkowania zostanie wyjaśniona w kolejnych punktach opisu algorytmu.
...
\SetKwProg{myproc}{Procedure}{}{}
\SetKwFunction{proc}{operator\<}
\Input{$a,b - pary instancji typu FeatureInstance $}
\Output{$ bool $}
