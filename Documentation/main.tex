...
Na Rysunku~\ref{chart:planesweep-datasize} przedstawiono jednak dla implementacji na GPU czasy wykonania samych obliczeń nie uwzględniając niezbędnych operacji związanych z przygotowaniem danych do przetwarzania na karcie graficznej. Na Rysunku \ref{chart:gpu-planesweep-data-preparation} porównano czasy wykonań części odpowiedzialnej za odkrywanie relacji sąsiedztwa w.w. implementacji bez oraz z~uwzględnieniem przygotowania danych. Łatwo można na nim zauważyć, jak duży może być nakład pracy związany z przygotowaniem danych który umożliwi przetwarzanie na GPU.
\begin{figure}[H]
\begin{tikzpicture}
\begin{axis}[
            legend style={at={(0.5,-0.25)},anchor=north,legend cell align=left},
