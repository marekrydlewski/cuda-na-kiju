...
Metoda odkrywania maksymalnych kolokacji w oparciu o graf rzadki i skondensowane drzewo (ang. \textit{sparse-graph and condensed tree-based maximal co-location algorithm}) została przedstawiona w pracy \cite{chinczyki} i miała na celu zmniejszenie czasu wykonywania odkrywania poprzez wyeliminowanie kosztów powodowanych przez operacje na drzewach typu \textit{branch chopping} czy \textit{grafting}, z których korzysta wiele dzisiejszych algorytmów wyszukiwania kolokacji. W porównaniu do innych metod, algorytm \textit{SGCT} ogranicza także zużycie pamięci operacyjnej, gdyż nie wymaga budowy tymczasowej struktury do przechowywania duplikatów instancji kolokacji.

Poszczególne kroki algorytmu zostaną opisane w kolejnych podrozdziałach.
